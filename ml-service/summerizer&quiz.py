...
# Convert PPT/PPTX → Text (using AI to extract text from presentation)
def ppt_to_text(ppt_file):
    try:
        ...
        # Use OpenAI to extract text from PPT
        # Since we can't directly read PPT, we'll use a workaround
        # For now, return a message asking user to provide text
        ...
        try:
            ...
            # If python-pptx is not installed, use AI to extract
...
# Extract text from file (PDF or PPT)
...
def extract_text():
    ...
    try:
        if filename.endswith('.pdf'):
            ...
            if text is None:
                # Fallback: Try to use AI to extract text
                # For now, return error with suggestion
                return jsonify({
                    "error": "PPT extraction requires python-pptx library. Please install it with: pip install python-pptx, or convert PPT to PDF first."
                }), 400
        else:
...
def ai_chat():
    ...
    context = data.get("context", "")  # Optional: subject or topic context
...
def analyze_difficulty():
    """Analyze difficulty of quiz questions"""
    data = request.get_json()
